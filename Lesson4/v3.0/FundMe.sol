...
error NotOwner(); //inside modifier onlyOwner we added error function, which helps gas
...
contract FundMe{
    ...
    // Becasue of gas efficiency reasons, we add constant keyboard
    ...
    //In order to be gas efficient, we can add immutable and constant keyword for the variables that we set once and never change again
    ...
    function fund() public payable{
        ...
        require(msg.value.getConversion() >= MIN_USD, "Send little more, like 1 ETH or 50 USD"); 
     ...
     modifier onlyOwner{
      ...
      if(i_owner==msg.sender) {revert NotOwner();}//this way saves gas.
     ...
     //If someone sended us native currency without using fund, we will not know who sended the money.
     //In order to add them even though they did not call fund function, we ill add receive and fallback functions
